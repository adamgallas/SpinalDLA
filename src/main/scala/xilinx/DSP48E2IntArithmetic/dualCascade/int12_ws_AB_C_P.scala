...
class int12_ws_AB_C_P(length: Int, width: Int) extends Component {
  ...
  for (i <- 0 until length) {
    ...
    ab_concat.set_pingpong_ab_concat_attr(attrs(i))
  ...
  for (i <- 0 until length) {
    ...
    ab_concat.assign_pingpong_ab_concat_ctrl(dsp48e2s(i), enPrefetchChain(i), enFetchChain(i))
